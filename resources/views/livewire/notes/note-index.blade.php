                    ...
                    @foreach($myNotes as $note)
                        ...
                        >
                            <div class="flex items-start justify-between">
                                <span class="text-base font-semibold text-zinc-900 dark:text-zinc-100 line-clamp-1">
                                    {{ $note->title }}
                                </span>
                                <div @click.stop>
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach($sharedNotes as $note)
                        ...
                        >
                            ...
                            <span class="mt-2 text-base font-semibold text-zinc-900 dark:text-zinc-100 line-clamp-1">
                                {{ $note->title }}
                            </span>
                            <flux:text class="mt-2 line-clamp-3 text-sm">{{ Str::limit(strip_tags($note->content), 150) }}</flux:text>
                            <flux:text class="mt-auto pt-3 text-xs text-zinc-400">{{ $note->updated_at->diffForHumans() }}</flux:text>
